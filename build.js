...
async function buildGraph() {
    ...
    // --- PHASE 5: Compile sw.js (PWA Service Worker) ---
    const globCacheFiles = (dir, base = '') => {
        let results = [];
        ...
        list.forEach(file => {
            ...
            if (stat && stat.isDirectory()) {
                results = results.concat(globCacheFiles(filePath, relativePath));
            } else {
                // Do not cache sw.js, system generated or hidden files
                if (file !== 'sw.js' && file !== '.git' && !file.startsWith('.') && !file.endsWith('.tmp') && !file.endsWith('.log')) {
    ...
    const assetsToCache = globCacheFiles(outputDir);
    // Add additional critical root route references
    assetsToCache.push('');
    assetsToCache.push('/');
    assetsToCache.push('index.html');
    
    // De-duplicate array
    const cacheList = Array.from(new Set(assetsToCache)).sort();
    
    const swContent = `// AUTO-GENERATED BY BUILD.JS - DO NOT EDIT DIRECTLY
const CACHE_NAME = 'neuron-iq-cache-${Date.now()}';
...
`;
    ...
    console.log(`[Neuron-IQ] Service Worker sw.js compiled! Cached ${cacheList.length} local files.`);
}
